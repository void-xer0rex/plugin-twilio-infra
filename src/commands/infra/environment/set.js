...
class InfraEnvironmentSet extends TwilioClientCommand {
  async run() {
    await super.run();
    const { args } = this.parse(InfraEnvironmentSet);
    const environment = args.environmentName;
    try {
      const deploymentEnvironments = readInfra();
      let twilioProject;
      for (const asid in deploymentEnvironments) {
      ...
      if (twilioProject) {
        ...
        if (twilioProject !== this.twilioClient.accountSid) {
          Printer.print(`Remember to switch to Twilio project ${twilioProject} before deploying the resources.`);
        }
      } else {
        Printer.print(
...
InfraEnvironmentSet.description = 'Set the deployment environment for the current project';
